...
s_region <- with(data_smol, summary(as.numeric(pid7_num) ~ region, fun=sf))

plot(s_region, which=1:3, pch=1:3, xlab='logit', main=' ', xlim=range(s[,3:4]))

#testing assumptions for region + urbancity 
s_region_city<- with(data_smol, summary(as.numeric(pid7_num) ~ region + urbancity, fun=sf))
...
relig_imp_model <- polr(pid7 ~ pew_religimp, data = data_smol, Hess=TRUE)

summary(relig_imp_model)

# Extracting coefficient information
coef_summary <- summary(relig_imp_model)$coef[1:3,]

# Names of the religions
religions <- sub("pew_religimp", "", rownames(coef_summary))
religions <- sub("religpew", "", religions)
coefficients <- as.numeric(coef_summary[, 1])
se <- as.numeric(coef_summary[, 2])
...
df <- df %>% mutate(Importance = case_when('Not at all important' %in% Religion ~ 'Not important',
                                           'Not too important' %in% Religion ~ 'Not important',
                                           'Not at all important' %in% Religion ~ 'Not important')) 
...
rf_model <- randomForest(pid7 ~ ., data = train_data, ntree = 100, importance = TRUE)
